...
htpasswd -c -b target/.htpasswd "${AK8S_ELASTIC_USER}" "${AK8S_ELASTIC_PASSWORD}"
...
kubectl create secret generic elasticsearch-password-secret --from-literal=user=${AK8S_ELASTIC_USER} --from-literal=password=${AK8S_ELASTIC_PASSWORD}
